...
# --- Eval loop ---
def run_eval(test_examples, tokenizer, model):
    results = []
...
if __name__ == "__main__":
    ...
    for fname in adapter_files:
        try:
            download_from_s3(f"{S3_PREFIX}/adapter/{fname}", f"adapter/{fname}")
    ...
    if baseline_path.exists():
        ...
        print("\n--- Before vs After ---")
        print(f"  {'metric':25s} {'baseline':>10} {'finetuned':>10} {'delta':>10}")
        print("  " + "-" * 60)
        for k in baseline_metrics:
            b = baseline_metrics[k]
            ft = metrics[k]
            print(f"  {k:25s} {b:>10.4f} {ft:>10.4f} {ft - b:>+10.4f}")

        wandb.log({f"delta/{k}": metrics[k] - baseline_metrics[k] for k in baseline_metrics})

    wandb.log(metrics)
    wandb.finish()

    # Save results
    with jsonlines.open("results/finetuned_results.jsonl", mode="w") as writer:
    ...
    # Upload to S3
    upload_to_s3("results/finetuned_results.jsonl", f"{S3_PREFIX}/results/finetuned_results.jsonl")
    upload_to_s3("results/finetuned_metrics.json",  f"{S3_PREFIX}/results/finetuned_metrics.json")
